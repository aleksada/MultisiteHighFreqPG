...
```{r simulate rainfall}
sims <- function(NT,NS,sigma21,beta1,b0,b1,alpha,nu,ut){
    error.sim <- matrix(0,NT,NS)
    distance <- as.matrix(dist(coord[1:NS,2:3], diag = TRUE, upper = TRUE))
    ...
    ut.sim <- matrix(rep(ut,each=1+NT/length(ut[,1])),ncol = 8)[1:NT,1:NS]
    for(t in 2: NT){
...
y.sim <- sims(NT,NS,sigma21,beta1,b0,b1,0,20,ut)
plot.ts(y.sim$y.sim)
```
...
y.sim.vis[,1] <- sims(NT,NS,sigma21,beta1,b0,b1,0,20,ut)$y.sim[,1]
y.sim.vis[,2] <- sims(NT,NS,sigma21,beta1,b0,b1,5,20,ut)$y.sim[,1]
y.sim.vis[,3] <- sims(NT,NS,sigma21,beta1,b0,b1,0,7,ut)$y.sim[,1]
y.sim.vis[,4] <- sims(NT,NS,sigma21,beta1,b0,b1,5,7,ut)$y.sim[,1]
y.sim.vis[,5] <- sims(NT,NS,sigma21,beta1,b0,b1,0,3,ut)$y.sim[,1]
y.sim.vis[,6] <- sims(NT,NS,sigma21,beta1,b0,b1,5,3,ut)$y.sim[,1]
...
```{r}
varlik.sp.var1.st.sim <- function(params, data, coord, ut, nu){
    NS <- ncol(data)
    NT <- nrow(data)
    ...
    for (s in 1:NS){
        ...
        u <- (ut.sim[index.zero + 1, s] - data[index.zero,] %*% B1[s,])/
...
varlik.sp.var1.norm.sim <- function(params, data, coord, ut){
    NS <- ncol(data)
    NT <- nrow(data)
...
est.sims<-function(y.sim, nu){
    ...
    #                    hessian = TRUE)
    cov.optim.rslt.st <- neldermead(x0=init,fn = varlik.sp.var1.st.sim,
                            lower = c(0,0,0,0,-10),upper = c(1,10,10,10,100),
                            data = y.sim, coord = coord, ut = ut, nu = nu)
    est_st <- cov.optim.rslt.st$par
    
    init_norm <- init[-5]
    cov.optim.rslt.norm <- neldermead(x0=init_norm,fn = varlik.sp.var1.norm.sim,
                            lower = c(0,0,0,0),upper = c(1,10,10,10),
                            data = y.sim, coord = coord, ut = ut)
    est_n<-cov.optim.rslt.norm$par
...
### Run the estimations
```{r fit}
y.sim <- sims(10000,3,sigma21,beta1,b0,b1,0,3,ut)$y.sim
est1 <- est.sims(y.sim=y.sim,nu=3)
y.sim2 <- sims(10000,3,sigma21,beta1,b0,b1,5,3,ut)$y.sim
...
y.sim5 <- sims(10000,3,sigma21,beta1,b0,b1,0,20,ut)$y.sim
