...
ut.fun <- function(data,sep){
    ...
    season.fit <- glm(binaryrain ~ sint[,1:i] + cost[,1:i] + 
                          sindt[,1:i] + cosdt[,1:i],family = "binomial")
    save.aic[i] <- AIC(season.fit)
}
H.best <- which(save.aic == min(save.aic))
season.fit.best <- glm(binaryrain ~ sint[,1:H.best] + cost[,1:H.best] + 
                           sindt[,1:H.best] + cosdt[,1:H.best], family="binomial")
...
init[3] <- 0.5
init[4] <- 0.5
init[5] <- 2 #positive skewness alpha from exploratory analysis
cov.optim.rslt.st <- bobyqa(x0=init,fn = varlik.sp.var1.st,
                            lower = c(0,0,0,0,-10),upper = rep(10,5),
...
#init[1] <- 0.1/(1/NS-0.1) #variance
init[1] <- 0.3
init[2] <- 0.2 #spatial range
init[3] <- 1
init[4] <- 1
cov.optim.rslt.norm <- neldermead(x0=init,fn = varlik.sp.var1.norm,
                            lower = c(0,0,0,0),upper = c(1,100,10,10),
